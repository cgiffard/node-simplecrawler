...
var FetchQueue  = require("./queue.js"),
    ...
    FetchAgent = require("./fetch-agent.js"),
...
var Crawler = function(initialURL) {
    ...
    /**
     * Controls what fetch agent to use, default is {@link https://github.com/node-fetch/node-fetch/blob/master/README.md|node-fetch}. This is useful if you want to configure
     * eg. a window.fetch.
     * @type {FetchAgent}
     */
...
/**
 * Generates a request object for fetch
 * @param  {QueueItem} queueItem The queue item for which a request object should be generated
 * @return {Object}              Returns a request object for fetch
 */
Crawler.prototype.getRequest = function(queueItem) {
    var crawler = this;
    ...
    headers["Host"] = queueItem.host + (queueItem.port && isStandardPort ? ":" + queueItem.port : "");
    ...
    if (queueItem.referrer) {
        headers["Referer"] = queueItem.referrer;
    }

    // Add cookie header from cookie jar if we're configured to
    // send/accept cookies
    if (crawler.acceptCookies && crawler.cookies.getAsHeader()) {
        headers["cookie"] = crawler.cookies.getAsHeader(queueItem.host, queueItem.path).join("; ");
    }
        ...
        crawler.cache.getCacheData(queueItem, function(cacheObject) {
            if (cacheObject) {
                ...
                if (cacheObject.lastModified) {
                    headers["If-Modified-Since"] = cacheObject.lastModified;
                }
            }
        });
    }
    ...
    if (crawler.customHeaders) {
        for (var header in crawler.customHeaders) {
            if (crawler.customHeaders.hasOwnProperty(header)) {
                headers[header] =  crawler.customHeaders[header];
            }
        }
    }

    var AbortController = crawler.fetchAgent.options.abortController;
    ...
    return {
        ...
        init: {
            method: "GET",
            redirect: crawler.fetchAgent.options.redirect,
            headers: headers,
            signal: signal,
            agent: agent,
            compress: crawler.decodeResponses
        },
...
Crawler.prototype.getRobotsTxt = function(url, callback) {
    ...
    var fetchRequest = crawler.getRequest(crawler.processURL(robotsTxtUrl.href()));
    ...
    crawler.fetchAgent.fetch(fetchRequest.input, fetchRequest.init).then(function (response) {
        ...
        if (response.status >= 200 && response.status < 300) {
            ...
            response.arrayBuffer().then(function (arrayBuffer) {
                ...
                return callback(new Error("Couldn't get response buffer from response object" + error));
            });
            ...
            try {
                redirectTarget = uri(response.headers.get("location"))
            ...
            errorMsg = util.format("Server responded with status %d when fetching robots.txt", response.status);
            callback(new Error(errorMsg));
        }
    }).catch(function (error) {
        callback(error);
...
/**
 * Gets headers as JSON from Header object recceived from fetch
 * @param {Headers} headerObj
 */
Crawler.prototype.getHeadersAsJson = function(headerObj) {
    var headers = {};
    for (var header of headerObj.entries()) {
 ...
 * been received, {@link Crawler.handleResponse#handleResponse} will handle the processing of
 * the resource data
...
Crawler.prototype.fetchQueueItem = function(queueItem) {
    ...
    crawler.queue.update(queueItem.id, {
        ...
        var fetchRequest = crawler.getRequest(queueItem),
        ...
        var fetchTimeout = setTimeout(function() {
            ...
            if (crawler.running && !queueItem.fetched) {
                ...
                crawler._openRequests.splice(
                    crawler._openRequests.indexOf(fetchRequest), 1);
            }
        ...
        crawler.fetchAgent.fetch(fetchRequest.input, fetchRequest.init).then(function (response) {
            clearTimeout(fetchTimeout);
            crawler.handleResponse(queueItem, fetchRequest, response, timeCommenced);
        }).catch(function (error) {

            if (queueItem.fetched) {
                return;
            }
            if (crawler._openRequests.indexOf(fetchRequest) > -1) {
                crawler._openRequests.splice(
                    crawler._openRequests.indexOf(fetchRequest), 1);
            }
            ...
            }, function(errorData, queueItem) {
                if (errorData) {
                    return crawler.emit("queueerror", errorData, queueItem);
                }
        ...
        crawler._openRequests.push(fetchRequest);
         ...
         */
        crawler.emit("fetchstart", queueItem, fetchRequest);
    });
...
/**
 * Handles processing of a resource after an initial HTTP response has been
 * received.
 ...
 * @param  {Request} request     An instance of [Request]{@link https://developer.mozilla.org/en-US/docs/Web/API/Request}
 * @param  {Response} response   An instance of [Response]{@link https://developer.mozilla.org/en-US/docs/Web/API/Response}
 * @param  {Date} [timeCommenced=Date.now()] Specifies at what time the request was initiated
 * @return {Crawler}                         Returns the crawler instance to enable chained API calls
 */
Crawler.prototype.handleResponse = function (queueItem, request, response, timeCommenced) {
    var crawler = this,
        dataReceived = false,
    ...
    crawler.queue.update(queueItem.id, {
        stateData: {
            ...
            headers:  crawler.getHeadersAsJson(response.headers)
        }
    }, function(error, queueItem) {
        if (error) {
            return crawler.emit("queueerror", error, queueItem);
        ...
        if (crawler.acceptCookies && response.headers.has("set-cookie")) {
            try {
                crawler.cookies.addFromHeaders(response.headers.get("set-cookie"));
            } catch (error) {
                /**
                 ...
                 */
                crawler.emit("cookieerror", queueItem, error, response.headers.get("set-cookie"));
            }
        }
        ...
        if (response.status >= 200 && response.status < 300) {
            // Depending on [redirect option]{@link https://developer.mozilla.org/en-US/docs/Web/API/Request/redirect}
            // behaviour of response will vary.
            if (response.redirected) {
                crawler.queue.update(queueItem.id, {
                    ...
                    redirectQueueItem = crawler.processURL(response.url, queueItem);
                    ...
                    // Remove this request from the open request map
                    crawler._openRequests.splice(
                        crawler._openRequests.indexOf(request), 1);
                });
            } else {
                        ...
                        crawler.queue.update(queueItem.id, {
                            ...
                            if (crawler.downloadUnsupported || crawler.mimeTypeSupported(contentType)) {
                                response.arrayBuffer().then(function (buffer) {
                                    ...
                                    // Set response buffer to empty
                                    crawler.emit("arraybuffererror", error, queueItem);
        ...
        // If we should queue a redirect
            ...
            crawler.queue.update(queueItem.id, {
                ...
                // Parse the redirect URL ready for adding to the queue...
                redirectQueueItem = crawler.processURL(response.headers.get("location"), queueItem);
        ...
        // Ignore this request, but record that we had a 404
            ...
            crawler.queue.update(queueItem.id, {
                 ...
                 * @param {Response} response             An instance of [Response]{@link https://developer.mozilla.org/en-US/docs/Web/API/Response}
                 */
                crawler.emit("fetch" + response.status, queueItem, response);
                ...
                if (/(gzip|deflate)/.test(queueItem.stateData.headers["content-encoding"])) {
                    zlib.unzip(responseBuffer, function(error, decompressedBuffer) {
                        if (error) {
                             ...
                             * @param {String|Buffer} responseBody    If {@link Crawler#decodeResponses} is true, this will be the decoded HTTP response. Otherwise it will be the raw response buffer.
                             * @param {Error} error                   If unzip failed due to any error.
 ...
 * @param {Response} response   The response object as returned by fetch API. [Refer]{@link https://developer.mozilla.org/en-US/docs/Web/API/Response}
