...
var Crawler = function(initialURL) {
    ...
    if (this.urlNormalize) {
        parsedURL = uri(initialURL).normalize();
    } else {
        parsedURL = uri(initialURL);
        parsedURL.normalizeProtocol();
        parsedURL.normalizeHostname();
        parsedURL.normalizePort();
    }
    ...
    /**
     * Controls URL use urijs.normalize().
     * @type {Boolean}
     */
...
Crawler.prototype.urlIsAllowed = function(url) {
    ...
    if (crawler.urlNormalize) {
        formattedURL = uri(url).normalize().href();
    } else {
        formattedURL = uri(url);
        formattedURL.normalizeProtocol();
        formattedURL.normalizeHostname();
        formattedURL.normalizePort();
        formattedURL = formattedURL.href();
    }
    ...
    var clientRequest = client.request(requestOptions, function(response) {
        if (response.statusCode >= 200 && response.statusCode < 300) {
            ...
            if (crawler.urlNormalize) {
                ...
                redirectTarget = uri(response.headers.location);
                redirectTarget =redirectTarget.absoluteTo(robotsTxtUrl);
                redirectTarget.normalizeProtocol();
                redirectTarget.normalizeHostname();
                redirectTarget.normalizePort();
            }
...
Crawler.prototype.processURL = function(url, referrer) {
    ...
    try {
        newUrl = uri(url).absoluteTo(referrer.url);

        if (crawler.urlNormalize) {
            newUrl = newUrl.normalize();
        } else {
            newUrl.normalizeProtocol();
            newUrl.normalizeHostname();
            newUrl.normalizePort();
        }

        if (crawler.urlNormalize && crawler.urlEncoding === "iso8859") {
        ...
        .reduce(function(list, URL) {
            ...
            try {
                if (crawler.urlNormalize) {
                    ...
                    URL = uri(URL);
                    URL =  URL.absoluteTo(queueItem.url || "");
                    URL.normalizeProtocol();
                    URL.normalizeHostname();
                    URL.normalizePort();
                    URL = URL.href();
                }
            } catch (e) {
...
Crawler.prototype.crawl = function() {
    ...
    crawler.queue.oldestUnfetchedItem(function(error, queueItem) {
        if (error) {
            ...
            var url = uri(queueItem.url);

            if (crawler.urlNormalize) {
                url = uri(queueItem.url).normalize();
            } else {
                url = uri(queueItem.url);
                url.normalizeProtocol();
                url.normalizeHostname();
                url.normalizePort();
            }
